...
@Component({
    selector: 'app-lead-tasks-dialog',
    templateUrl: './lead-tasks-dialog.component.html',
    styleUrls: ['./lead-tasks-dialog.component.scss'],
    providers: [DatePipe],
    standalone: false
})

export class LeadTasksDialogComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<LeadTasksDialogComponent>,
  ...
  updateFollowup() {
    ...
    let task = new LeadTask;
    task.id = this.leadTask.id;
    task.title = this.leadTaskForm.controls['title'].value;
    task.description = this.leadTaskForm.controls['description'].value;
    task.task_type = this.leadTaskForm.controls['task_type'].value;
    task.priority = this.leadTaskForm.controls['priority'].value;
    task.status = this.leadTaskForm.controls['status'].value;
    task.due_date = this.datepipe.transform(this.leadTaskForm.controls['due_date'].value, 'yyyy-MM-dd');

    this.leadService.updateLeadTask(task)
      .subscribe(() => {
        this.leadTaskFormDisabled = false;
        this.dialogRef.close();
