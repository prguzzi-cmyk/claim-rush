...
export default async function WaterLeadsPage() {
  let leads: PaginatedLeads = { items: [], total: 0, page: 1, size: 50, pages: 0 };

  try {
    leads = await fetchAPI<PaginatedLeads>(
      "/api/v1/leads?size=50&peril=water"
    );
  } catch (error) {
    console.error("Failed to fetch water leads:", error);
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {leads.total} total water damage leads
          </p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {leads.items.length === 0 ? (
            <p className="text-sm text-muted-foreground">No water leads found.</p>
          ) : (
