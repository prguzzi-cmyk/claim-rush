...
function SatelliteConsole({ scanning, onScan, address, setAddress }) {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            Backend Pending · Scan Simulated
          </div>
        </div>
...
export default function RoofIntel() {
  ...
  function startScan(addr) {
    if (!addr.trim() || scanning) return;
    setScanning(true);
    // Backend not yet wired; simulate scan completion. When the real
    // /v1/roof-intel/lookup endpoint ships, swap this for an apiFetch
    // and surface the response in a results panel.
    setTimeout(() => setScanning(false), 2600);
  }

  // Derived metrics (from demo data today; live data when backend ships).
  ...
  return (
    <div style={{ maxWidth: 1200 }}>
      ...
      <SectionStrip
        label={`Operational Targets · ${DEMO_TARGETS.length}`}
        color={COPPER}
        right={
          <span style={{
            display: "inline-flex", alignItems: "center", gap: 5,
