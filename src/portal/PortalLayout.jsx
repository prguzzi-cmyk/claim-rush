...
const ALL_NAV_ITEMS = [
  { to: "/portal", label: "Dashboard", icon: "\u{1F4CA}", end: true, page: "dashboard" },
  // Field user items (labels change per role context, but route is same)
  { to: "/portal/fire-leads", label: "Leads", icon: "\u{1F525}", page: "fire-leads", altLabels: { RVP: "Team" } },
  { to: "/portal/earnings", label: "Earnings", icon: "\u{1F4B0}", page: "earnings" },
  // CP-specific
  { to: "/portal/territory-revenue", label: "Territory", icon: "\u{1F5FA}\uFE0F", page: "territory-revenue" },
  // Home office admin
  { to: "/portal/payout-runs", label: "Payouts", icon: "\u{1F4B3}", page: "payout-runs" },
  { to: "/portal/billing", label: "Subscriptions", icon: "\u{1F4B5}", page: "billing" },
];

const ROLE_COLORS = { home_office: "#E05050", CP: "#00E6A8", RVP: C.gold, agent: C.blue };

function PortalInner() {
  const [axisOpen, setAxisOpen] = useState(false);
  const { liveContext, addSession, userRole, setUserRole, permissions } = useAxisContext();
  const navItems = ALL_NAV_ITEMS.filter(item => permissions.pages.includes(item.page)).map(item => ({
    ...item,
        ...
        <div style={{ marginTop: "auto", borderTop: `1px solid ${C.border}` }}>
          ...
          <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {["home_office", "CP", "RVP", "agent"].map(role => (
                <button
                  key={role}
                  onClick={() => setUserRole(role)}
                  style={{
                    flex: 1, padding: "4px 0",
                    background: userRole === role ? `${ROLE_COLORS[role]}15` : "transparent",
                    border: `1px solid ${userRole === role ? `${ROLE_COLORS[role]}40` : "rgba(255,255,255,0.06)"}`,
                    borderRadius: 4,
                    color: userRole === role ? ROLE_COLORS[role] : "rgba(255,255,255,0.35)",
                    fontSize: 10, fontWeight: 700, letterSpacing: 0.5,
                    cursor: "pointer", fontFamily: "'Courier New', monospace",
                    transition: "all 0.2s",
                  }}
                >
                  {role === "home_office" ? "HO" : role.toUpperCase()}
                </button>
              ))}
            </div>
          </div>
