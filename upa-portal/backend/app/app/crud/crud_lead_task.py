...
"""CRUD operations for the lead task model"""
...
from app.schemas import LeadTaskCreateDB, LeadTaskUpdate


class CRUDLeadTask(CRUDBase[LeadTask, LeadTaskCreateDB, LeadTaskUpdate]):
    def update(
        self,
        db_session: Session,
        *,
        obj_id: UUID,
        obj_in: LeadTaskUpdate | dict[str, Any]
    ) -> LeadTask:
        ...
        with db_session as session:
            task_obj: LeadTask = session.scalars(
                select(LeadTask).where(LeadTask.id == obj_id)
            ).first()
...
lead_task = CRUDLeadTask(LeadTask)
