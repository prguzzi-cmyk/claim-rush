    ...
    def list_claims(self, db: Session) -> list[dict[str, Any]]:
        ...
        for c in claims:
            out.append({
                ...
                "carrier_estimate_amount": _round(c.carrier_estimate_amount) if c.carrier_estimate_amount is not None else None,
                "estimate_divergence_flagged": bool(c.estimate_divergence_flagged),
                "estimate_divergence_percentage": float(c.estimate_divergence_percentage) if c.estimate_divergence_percentage is not None else None,
                "estimate_divergence_dollars": _round(c.estimate_divergence_dollars) if c.estimate_divergence_dollars is not None else None,
